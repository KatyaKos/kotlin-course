...
class Visitor : FunBaseVisitor<Ast>() {
    ...
    override fun visitRelationExpression(ctx: FunParser.RelationExpressionContext?): Ast =
            RelationExpression(visit(ctx!!.left) as Expression, ctx.operation.text, visit(ctx.right) as Expression)

    override fun visitEqualityExpression(ctx: FunParser.EqualityExpressionContext?): Ast =
            EqualityExpression(visit(ctx!!.left) as Expression, ctx.operation.text, visit(ctx.right) as Expression)

    override fun visitOrAndExpression(ctx: FunParser.OrAndExpressionContext?): Ast =
            orAndExpression(visit(ctx!!.left) as Expression, ctx.operation.text, visit(ctx.right) as Expression)
